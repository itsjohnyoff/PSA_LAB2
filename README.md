...
This repository contains ten laboratory assignments for the Probability and Statistics (PSA) course. Each problem uses Python to combine analytical computation with Monte Carlo simulation, allowing theoretical probability results to be verified empirically.
...
## Problem 9 – Random Variables Game

### Problem Statement

A random number $X$ is chosen uniformly from $[0,1]$. Then, a sequence of independent random variables $Y_1, Y_2, Y_3, \dots$ is generated, each uniformly distributed on $[0,1]$. The game ends at the first index $i$ such that $Y_i > X$. The player receives $(i - 1)$ dollars. The goal is to determine the fair entrance fee for the game and verify the result through simulation.

### Mathematical Background

* **Threshold Selection**: $X \sim U(0,1)$ represents the threshold.
* **Stopping Time**: Given $X = x$, each trial $Y_i > x$ has a success probability $p = 1 - x$. The stopping time $N$ is geometric with parameter $p$:
  $$P(N = k \mid X = x) = (1-x)x^{k-1}$$
* **Conditional Expected Payout**: The expected payout $N-1$ conditioned on $X = x$ is:
  $$E[N - 1 \mid X = x] = E[N \mid X = x] - 1 = \frac{1}{1-x} - 1 = \frac{x}{1-x}$$
* **Expected Value / Fair Entrance Fee**: Integrating over the uniform density of $X$:
  $$E[\text{payout}] = \int_0^1 \frac{x}{1-x} \, dx = \lim_{t \to 1^-} \left[ -x - \ln(1-x) \right]_0^t = +\infty$$
  Since the expected winnings are infinite, the fair entrance fee is mathematically infinite.

### Methodology

1. **Simulation Procedure**: For $M$ independent game runs, we sample $X \sim U(0,1)$ and then sample $N \sim \text{Geometric}(1-X)$ to determine the stopping time.
2. **Payout Computation**: The payout for each game is computed as $N - 1$.
3. **Monte Carlo Estimation**: The expected payout is estimated as the mean of the payouts across all games. We run simulations with scaling iteration counts ($1,000$, $10,000$, $100,000$, $1,000,000$) to observe how the average payout behaves.

### Output

Running `problem9.py` prints:

```
Random Variables Game Simulation

Games:     1,000 | Average payout: $6.4460
Games:    10,000 | Average payout: $19.3774
Games:   100,000 | Average payout: $11.1671
Games: 1,000,000 | Average payout: $14.9706

Conclusion:
  The theoretical expected payout is infinite (integral of x / (1-x) from 0 to 1).
  As the number of games increases, the empirical average grows because the probability
  of sampling an X close to 1 increases, resulting in very large payouts.
```

### Interpretation

Because the theoretical expected payout is infinite, the empirical average does not converge to a finite number as the number of games increases. Instead, as the sample size $M$ grows, the probability of sampling an $X$ value extremely close to $1$ increases. These rare, extreme samples result in massive stopping times (payouts), causing the running average to remain unstable and show an overall upward trend over very large sample sizes, demonstrating the infinite mean property of the game.

---

## Problem 10 – Important Distributions

### Problem Statement

Jora Petrovici never pays for the trolleybus ride. A valid ticket costs 7 lei. The probability of being caught by a controller on any ride is 0.05. The fines for offenses are progressive: 50 lei for the first offense, 200 lei for the second offense, and 300 lei for all subsequent offenses. Additionally, with probability 0.02, the tax collector is a hairy muscular guy, in which case Jora immediately pays the normal ticket price of 7 lei. Assuming Jora rides the trolleybus twice per day for an entire year (365 days), we compare the expected annual cost of fare evasion with the cost of always buying a ticket.

### Mathematical Background

* **Annual Rides**: The total number of rides in a year is $N = 365 \times 2 = 730$.
* **Ticket Purchases**: With probability $p_m = 0.02$, Jora meets the muscular collector and buys a ticket. The expected number of ticket purchases is $E[\text{tickets}] = 730 \times 0.02 = 14.6$, costing $14.6 \times 7 = 102.2$ lei.
* **Offenses**: If no muscular collector is met, Jora evades fare and is caught with probability 0.05. The overall probability of being caught on a ride is $p_f = (1 - 0.02) \times 0.05 = 0.049$. The expected number of offenses is $E[\text{offenses}] = 730 \times 0.049 = 35.77$.
* **Cumulative Fines**: The number of offenses $K$ follows a Binomial distribution:
  $$K \sim \text{Binomial}(730, 0.049)$$
  Since $P(K < 3) \approx 0$, the expected fine cost is:
  $$E[\text{Fine Cost}] \approx 300 E[K] - 350 = 300(35.77) - 350 = 10,381 \text{ lei}$$
* **Total Expected Annual Cost**: Jora's expected total annual cost is $102.2 + 10,381 = 10,483.2$ lei.
* **Student Cost**: A student who always buys a ticket pays $730 \times 7 = 5,110$ lei.

### Methodology

1. **Vectorized Annual Simulation**: We simulate $100,000$ years in parallel.
2. **Binomial Modeling**: For each year, we sample the number of ticket purchases $T \sim \text{Binomial}(730, 0.02)$, and then sample the number of offenses $O \sim \text{Binomial}(730 - T, 0.05)$.
3. **Escalating Penalty Application**: Masking is used to apply the progressive fine rates:
   - 0 offenses: 0 lei
   - 1 offense: 50 lei
   - 2 offenses: 250 lei
   - $\ge 3$ offenses: $300 \times O - 350$ lei
4. **Estimation**: The expected annual cost is computed as the average total cost over all simulated years.

### Output

Running `problem10.py` prints:

```
Simulating 100,000 years of trolleybus rides...

--- Results ---
Expected annual cost for Jora:      10487.87 lei
Annual cost for law-abiding student: 5110.00 lei

--- Conclusion ---
Fare evasion is not advantageous. Jora pays 5377.87 lei more on average.
```

### Interpretation

Fare evasion under this progressive fine system is highly disadvantageous. Over the course of 730 rides, Jora is caught on average 35.77 times. Because the fine increases to 300 lei after only the second offense, the progressive system acts as a powerful penalty for repeat evasion, making the long-term expected cost of fare evasion more than double the cost of buying a ticket every ride.
...
├── problem9.py                # Random variables game
├── problem10.py               # Important distributions (trolleybus fare evasion)
